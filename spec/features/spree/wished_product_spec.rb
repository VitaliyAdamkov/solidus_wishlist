RSpec.feature 'Wished Product', :js do
  ...
  context 'delete' do
    ...
    scenario 'remove first wishlist with multiple wished products while maintaining ordering by date added' do
      wished_products = [
        create(:wished_product, wishlist: wishlist),
        create(:wished_product, wishlist: wishlist),
        create(:wished_product, wishlist: wishlist)
      ]
      wished_product = wished_products[0]

      visit spree.wishlist_path(wishlist)

      wp_path = spree.wished_product_path(wished_product)
      delete_link = first(:link, "Remove from wishlist")
      delete_link.click
